...
// OAuth Routes
app.get('/auth/twitter/authorize', async (c) => {
...
app.get('/auth/twitter/callback', async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare(
      ...
      1, // You'll need to get actual user ID from session
      'twitter',
      userData.data.id,
      tokens.access_token,
...
// LinkedIn OAuth
app.get('/auth/linkedin/authorize', async (c) => {
...
// Facebook OAuth
app.get('/auth/facebook/authorize', async (c) => {
...
export default app;// OAuth Handlers for All Social Media Platforms
